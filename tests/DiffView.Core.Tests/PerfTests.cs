...
public sealed class PerfTests(ITestOutputHelper output)
{
    ...
    public void Build_on_a_similar_pair(int lines)
    {
        ...
        DiffBuildResult warm = DiffDocumentBuilder.Build(left, right, CancellationToken.None);
    ...
    public void Search_on_the_10k_pair()
    {
        ...
        Stopwatch stopwatch = Stopwatch.StartNew();
        FindResult literal = DiffSearch.Find(document, leftText, rightText, "alpha", CancellationToken.None);
        stopwatch.Stop();
        ...
        stopwatch.Restart();
        FindResult regex = DiffSearch.Find(document, leftText, rightText, @"\b(alpha|beta)\b", CancellationToken.None, new FindOptions { UseRegex = true, MaxMatches = 100_000 });
        stopwatch.Stop();
