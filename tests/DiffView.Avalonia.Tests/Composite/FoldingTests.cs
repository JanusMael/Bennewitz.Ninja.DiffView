...
public sealed class FoldingTests
{
    ...
    /// <summary>
    /// Blocks of each shape, so the boundary rule has all of its cases: a modification, which
    /// leaves both sides a line; an insertion and a deletion, which each leave one side padding;
    /// and a one-sided block at the very end, whose gap rides on the other side's last line.
    /// </summary>
