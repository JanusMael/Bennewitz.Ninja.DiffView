...
| 2 Theme-key audit and exhaustive dictionaries | in progress | Library core landing bottom-up, 45 unit tests: colour model + WCAG contrast; consumer reference scanner; variant-aware definition scanner; ResourceInclude graph resolver; variant-map parser; per-variant `ThemeInventory` with alias resolution and variant inheritance. Verified against real Semi: 7 variants, 0 unresolved includes, no `System*` keys. **Known gap:** Semi selects its Light/Dark palettes through code-behind dictionary elements (`<semi:Light/>` mapped by `x:Class`), which the resolver does not yet follow, so Light/Dark under-resolve colours; code-behind resolution is the next increment. Then: findings, `docs/theme-audit.md`, Fluent→Semi compat, DiffView.* tokens, reference-trait tests |
