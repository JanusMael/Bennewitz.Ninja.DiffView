...
two assemblies at 975 and 323 lines with different content, where only its owner knows which is
canonical; and [#47](https://github.com/JanusMael/ClaudeForge/issues/47), four wall-clock
dependencies in `MainWindowViewModel` — including a two-second watcher-suppression window no test
can assert either half of, and a 150 ms debounce with an already-documented race — which the
`TimeProvider` shape of #48 would make testable.
