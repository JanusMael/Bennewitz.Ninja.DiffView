...
| `ShowHeaders` and `ShowStatusStrip` are applied on **both** paths too, through one `ApplyChromeVisibility` per view rather than two copies | A host that sets either in XAML is ignored | `SideBySideDiffView.ApplyChromeVisibility`, `InlineDiffView.ApplyChromeVisibility`, called from `OnApplyTemplate` and from the `ShowHeadersProperty`/`ShowStatusStripProperty` branch; tests `ChromeToggleTests.The_headers_flag_set_before_the_template_applies_still_takes`, `The_status_strip_flag_set_before_the_template_applies_still_takes` |
| **`ShowBanner` is NOT applied that way, and must never be.** The banner's visibility belongs to the `:banner-none` pseudo-class, so the toggle is a disjunct inside `UpdatePseudoClasses` and nothing writes `IsVisible` on `PART_Banner` | A code write lands at `LocalValue`, outranks the style for the life of the control, and strands an empty banner on screen the next time a build has nothing to say | `SideBySideDiffView.UpdatePseudoClasses`, `InlineDiffView.UpdatePseudoClasses`; test `ChromeToggleTests.A_permitted_banner_with_nothing_to_say_stays_hidden`, which is the only test that separates this from the mechanism it replaced |
